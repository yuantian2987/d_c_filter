// dc_filter.cpp : Defines the entry point for the console application.
//
...
void serv_app_msg_dc_filter(INT16 *databufferIn_l,INT16 *databufferIn_r,INT16 len)
{
    ...
    if(len > 0) {
        ...
        for(j = 0; j < len; j++) {
            ...
            if(++dcf_count_l1 == SSA_FIRST_ORDER_CNT) {
		  ...
		  dcf_count_l1=0;
		  printf("%d \n",dcf_sum_l2);
                if(++dcf_count_l2 == SSA_SECOND_ORDER_CNT) {
...
int main(int argc, char* argv[])
{
	...
	int cnt,n;
	fp_in  = fopen("dc.bin","rb");//48_1K_16bit.bin 44_1_1k_16bit.bin 96_1k_16bit.bin resampe_test_8K.bin 16k_16bit.bin Aa22050-l-channel.pcm
	if (fp_in == NULL)
	{
		printf("aaaaab\n");
		return -1;
	...
	while(1)
	{
		...
		{
			serv_app_msg_dc_filter(app_in_buff_left,app_in_buff_right,n);
